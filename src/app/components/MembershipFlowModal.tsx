...
import {
  Check, Star, Sparkles, X, Medal,
  ArrowLeft, ShieldCheck, FileText, CreditCard, Lock,
  Crown, Building2, Award, Users, Car,
  UserCheck, ChevronRight, Zap,
} from 'lucide-react';
...
const INDIVIDUAL_TIERS: TierData[] = [
  ...
  },
  {
    ...
    features: [
      ...
      '1 free upgrade to Premiere Suite per year',
      'Paid upgrade to Premiere Suite: 40% off',
      'Flying Guest: 40% off',
      'Non-flying Guest: 40% off',
      'In-town Limousine Transfer: 40% off (1 limousine)',
  ...
  },
  {
    ...
    features: [
      ...
      '5 free upgrades to Premiere Suite per year',
      'Paid upgrade to Premiere Suite: 40% off',
      'Flying Guest: 40% off',
      'Non-flying Guest: 40% off',
      'In-town Limousine Transfer: 40% off (1 limousine)',
...
const CORPORATE_TIERS: TierData[] = [
  ...
  },
  {
    ...
    features: [
      ...
      'Shared by up to 5 Executive Nominees',
      '10 free upgrades to Premiere Suite per year',
      'Free 1 non-flying guest per Lounge Deluxe Entry',
      'Paid upgrade to Premiere Suite: 40% off',
      'Flying Guest: 40% off',
      'Non-flying Guest: 40% off',
...
const termsAndConditions = [
  ...
  },
  {
    ...
    body: 'Personal data collected during the membership application and throughout the membership period will be used solely for the purpose of administering your membership and improving our services. Data will be handled in accordance with the Personal Data (Privacy) Ordinance (Cap. 486) of Hong Kong.',
  },
  {
...
type FlowView = 'packages' | 'checkout' | 'success';
...
interface Props {
  member: MemberInfo;
  onClose: () => void;
  onSuccess?: (tier: { tierId: string; tierName: string; memberType: string; price: number; currency: string }) => void;
}

// ── Component ─────────────────────────────────────────────────────────────────

export function MembershipFlowModal({ member, onClose, onSuccess }: Props) {
  const { colors, mode } = useTheme();
  const [view, setView] = useState<FlowView>('packages');
  ...
  const headerBorderBottom = isDark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(200,199,190,0.4)';
  const memberChipBg = isDark ? 'rgba(220,181,21,0.12)' : 'rgba(220,181,21,0.1)';
  const indTabBg = activeTab === 'Individual' ? tabActiveBg : tabInactiveBg;
  const indTabBorder = activeTab === 'Individual' ? '1px solid rgba(220,181,21,0.45)' : tabInactiveBorder;
  const indTabColor = activeTab === 'Individual' ? 'rgb(220,181,21)' : subText;
  const corpTabBg = activeTab === 'Corporate' ? tabActiveBg : tabInactiveBg;
  const corpTabBorder = activeTab === 'Corporate' ? '1px solid rgba(220,181,21,0.45)' : tabInactiveBorder;
  const corpTabColor = activeTab === 'Corporate' ? 'rgb(220,181,21)' : subText;
  const indBadgeCountBg = activeTab === 'Individual' ? 'rgba(220,181,21,0.2)' : isDark ? 'rgba(255,255,255,0.08)' : 'rgba(200,199,190,0.4)';
  const corpBadgeCountBg = activeTab === 'Corporate' ? 'rgba(220,181,21,0.2)' : isDark ? 'rgba(255,255,255,0.08)' : 'rgba(200,199,190,0.4)';
  const agreeBtnBg = agreedToTerms ? 'rgba(220,181,21,0.08)' : sectionBg;
  const agreeBtnBorder = agreedToTerms ? '1px solid rgba(220,181,21,0.4)' : sectionBorder;
  const payBtnCursor = agreedToTerms ? 'pointer' : 'not-allowed';
  const payBtnShadow = agreedToTerms ? '0 8px 24px rgba(220,181,21,0.35)' : 'none';
  ...
  const memberFullName = `${member.title} ${member.firstName} ${member.lastName}`;
  ...
  function renderSuccess() {
    ...
    const orderRef = `MBR-${Date.now().toString().slice(-8)}`;
    const validUntil = new Date(new Date().setFullYear(new Date().getFullYear() + 1)).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' });

    const summaryRows = [
      { label: 'Member', value: memberFullName },
    ...
    return (
      <div className="max-w-lg mx-auto py-10 flex flex-col items-center text-center">
        <div
        ...
        <p className="text-2xl mb-2" style={{ color: pageText }}>Membership Activated!</p>
        <p className="text-sm mb-2 max-w-xs" style={{ color: subText }}>
          <strong style={{ color: 'rgb(220,181,21)' }}>{selectedTier.name} {selectedTier.memberType} Membership</strong>
          {' '}has been assigned to
        </p>
        <p className="text-sm mb-8" style={{ color: pageText }}>{memberFullName}</p>
    ...
    return (
      <div className="pb-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-2xl" style={{ color: pageText }}>Complete Your Purchase</p>
          <p className="text-sm mt-1" style={{ color: subText }}>Assigning membership to <strong style={{ color: pageText }}>{memberFullName}</strong></p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background: sectionBg, border: sectionBorder }}>
              <div className="overflow-y-auto p-5 space-y-5" style={{ maxHeight: '320px' }}>
            ...
            <button
              ...
              style={{ background: agreeBtnBg, border: agreeBtnBorder }}
            >
          ...
          <div className="space-y-4">
            <div className="rounded-2xl overflow-hidden" style={{ background: sectionBg, border: sectionBorder }}>
              ...
              <div className="p-5">
                ...
                <div className="rounded-lg px-3 py-2 mb-4 flex items-center gap-2" style={{ background: memberChipBg, border: '1px solid rgba(220,181,21,0.2)' }}>
                  <p className="text-xs" style={{ color: subText }}>For: <strong style={{ color: pageText }}>{memberFullName}</strong></p>
                </div>
                ...
                <ul className="space-y-2 mb-5">
                  ...
                  {selectedTier.features.length > 4 && (
                    <li className="text-xs" style={{ color: subText }}>+{selectedTier.features.length - 4} more benefits</li>
                  )}
                </ul>
    ...
    return (
      <div className="pb-8">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-2xl mb-2" style={{ color: pageText }}>Choose a Membership</p>
          <p className="text-sm max-w-md mx-auto" style={{ color: subText }}>
            Select a membership plan to assign to <strong style={{ color: pageText }}>{memberFullName}</strong>
          </p>
        </div>
        ...
        <div className="flex gap-3 mb-8 justify-center flex-wrap">
          <button
            ...
            style={{ background: indTabBg, border: indTabBorder, color: indTabColor }}
          >
            <Medal className="w-4 h-4" />
            Individual Membership
            <span className="text-xs px-1.5 py-0.5 rounded-md" style={{ background: indBadgeCountBg }}>4-tier</span>
          </button>
          <button
            onClick={handleTabCorporate}
            className="px-6 py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center gap-2"
            style={{ background: corpTabBg, border: corpTabBorder, color: corpTabColor }}
          >
            <Building2 className="w-4 h-4" />
            Corporate Membership
            <span className="text-xs px-1.5 py-0.5 rounded-md" style={{ background: corpBadgeCountBg }}>3-tier</span>
          </button>
        </div>
        ...
        <div className={`grid ${gridCols} gap-5 mb-10`}>
          {displayTiers.map((tier) => {
            ...
            const badgeHighlightStyle = { background: 'rgba(220,181,21,0.2)', color: 'rgb(220,181,21)', border: '1px solid rgba(220,181,21,0.4)' };
            const badgeNormalStyle = { background: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(200,199,190,0.5)', color: subText, border: cardBorder };
            const badgeStyle = tier.highlight ? badgeHighlightStyle : badgeNormalStyle;
            const upgradeIconColor = tier.freeUpgradesPerYear === 'NA' ? subText : tier.accentColor;
            const upgradeStatBg = isDark ? 'rgba(255,255,255,0.04)' : 'rgba(231,230,221,0.35)';
            const upgradeStatBorder = isDark ? '1px solid rgba(255,255,255,0.06)' : '1px solid rgba(200,199,190,0.35)';

            function handleSelect() { handleSelectTier(tier); }
            ...
            return (
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-1">
                  {tier.badge && (
                    <div className="mb-3">
                      <span className="text-xs px-2.5 py-1 rounded-full" style={badgeStyle}>{tier.badge}</span>
                    </div>
                  )}
                  <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="space-y-2 mb-4">
                    ...
                    <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: upgradeStatBg, border: upgradeStatBorder }}>
                      <Crown className="w-3.5 h-3.5 shrink-0" style={{ color: upgradeIconColor }} />
        ...
        {/* FAQ */}
        <div>
          <p className="text-lg mb-5 text-center" style={{ color: pageText }}>Frequently Asked Questions</p>
          <div className="space-y-3 max-w-2xl mx-auto">
            {faqItems.map((item, idx) => {
              const isOpen = openFaq === idx;
              const itemBg = isOpen ? faqOpenBg : faqBg;
              const chevronRotate = isOpen ? 'rotate(90deg)' : 'rotate(0deg)';
              function handleToggle() { handleFaqToggle(idx); }
              return (
                <div key={idx} className="rounded-xl overflow-hidden transition-all duration-200" style={{ background: itemBg, border: faqBorder }}>
                  <button className="w-full flex items-center justify-between px-5 py-4 text-left" onClick={handleToggle}>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-start justify-center z-50 p-4 overflow-y-auto">
      <div
        className="relative w-full max-w-5xl my-8 rounded-2xl shadow-2xl flex flex-col"
        style={{ background: modalBg, border: modalBorder }}
      >
        {/* Sticky Header */}
        <div className="flex items-center justify-between px-6 py-4 shrink-0" style={{ borderBottom: headerBorderBottom }}>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[rgb(220,181,21)] to-[rgb(180,141,11)] flex items-center justify-center">
              <Sparkles className="w-4 h-4 text-white" />
            </div>
            <div>
              <p className="text-sm font-semibold" style={{ color: pageText }}>Join Membership</p>
              <p className="text-xs" style={{ color: subText }}>{memberFullName}</p>
            </div>
          </div>
          {view !== 'success' && (
            <button
        ...
        {/* Scrollable Body */}
        <div className="px-6 pt-6 overflow-y-auto" style={{ maxHeight: 'calc(90vh - 72px)' }}>
