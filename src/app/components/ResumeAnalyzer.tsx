  ...
  const analyzeFiles = async (files) => {
    ...
    try {
      for (const file of files) {
        const { jobId, resumeId } = await atsApi.uploadResume(
          file,
          selectedJobProfile,
        );
    ...
    const poll = async () => {
      try {
        ...
        if (status.status === "completed") {
          const result =
            await atsApi.getAnalysisResult(resumeId);
        ...
        setError(
          `Failed to get analysis status for ${fileName}`,
        );
      }
    };
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="w-16 h-16 bg-gradient-to-br from-[rgb(220,181,21)] to-[rgb(180,141,11)] rounded-2xl flex items-center justify-center shadow-lg">
          ...
          <div>
            ...
            <p className="text-gray-600 mb-4">
              Drag & drop your resume files here, or click to
              browse
            </p>
            <p className="text-sm text-gray-500">
              Supports PDF, DOC, and DOCX files • Max 10MB per
              file
            </p>
          </div>

          <button className="px-6 py-3 bg-gradient-to-r from-[rgb(220,181,21)] to-[rgb(180,141,11)] text-white rounded-xl hover:shadow-lg transition-all duration-200 hover:-translate-y-1">
            Choose Files
          </button>
        </div>
      </div>
            ...
            {uploadedFiles.map((file, index) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-gradient-to-br from-[rgb(220,181,21)] to-[rgb(180,141,11)] rounded-lg flex items-center justify-center">
                    <FileText className="w-5 h-5 text-white" />
                  </div>
                  <div>
                    <p className="text-gray-800 truncate max-w-48">
                      {file.name}
                    </p>
          ...
          {analysisResults.map((result) => (
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-gradient-to-br from-[rgb(220,181,21)] to-[rgb(180,141,11)] rounded-xl flex items-center justify-center">
                ...
                <div className="flex items-center gap-4">
                  <div className="text-center">
                    <div
                      className={`text-3xl ${getScoreColor(result.analysis.overallScore)}`}
                    >
                      {result.analysis.overallScore}%
                    </div>
                    <div
                      className={`text-sm ${getScoreColor(result.analysis.overallScore)}`}
                    >
                      {getScoreLabel(
                        result.analysis.overallScore,
                      )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  ...
                  <p className="text-gray-800">
                    {result.analysis.personalInfo.name}
                  </p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <h4 className="text-sm text-gray-600 mb-2">
                  ...
                  <p className="text-gray-800 truncate">
                    {result.analysis.personalInfo.email}
                  </p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <h4 className="text-sm text-gray-600 mb-2">
                  ...
                  <p className="text-gray-800">
                    {result.analysis.personalInfo.phone}
                  </p>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <h4 className="text-sm text-gray-600 mb-2">
                  ...
                  <p className="text-gray-800">
                    {result.analysis.personalInfo.location}
                  </p>
                </div>
              </div>
                ...
                {result.analysis.sections.map(
                  (section, index) => (
                    ...
                    >
                      ...
                      <div
                        className={`text-xl ${getScoreColor(section.score)}`}
                      >
                        {section.score}%
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                        <div
                          className="bg-gradient-to-r from-[rgb(220,181,21)] to-[rgb(180,141,11)] h-2 rounded-full transition-all duration-300"
              ...
              {/* Job Match */}
              <div className="bg-gradient-to-r from-[rgb(220,181,21)]/20 to-[rgb(180,141,11)]/20 border border-[rgb(220,181,21)]/30 rounded-xl p-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div
                      className={`text-2xl ${getScoreColor(result.analysis.jobMatch.matchPercentage)}`}
                    >
                  ...
                  <div>
                    ...
                    <ul className="text-sm text-gray-700 space-y-1">
                      {result.analysis.jobMatch.recommendations
                        .slice(0, 2)
                        .map((rec, index) => (
                          <li
                            key={index}
      ...
      {uploadedFiles.length === 0 &&
        analysisResults.length === 0 && (
          <div className="text-center py-12">
            <div className="w-24 h-24 bg-gradient-to-br from-[rgb(220,181,21)]/20 to-[rgb(180,141,11)]/20 rounded-full flex items-center justify-center mx-auto mb-4">
              <FileSearch className="w-12 h-12 text-[rgb(220,181,21)]" />
            </div>
            <h3 className="text-xl text-gray-800 mb-2">
              No Resumes Uploaded
            </h3>
            <p className="text-gray-600">
              Upload your first resume to get started with
              AI-powered analysis.
            </p>
          </div>
        )}
